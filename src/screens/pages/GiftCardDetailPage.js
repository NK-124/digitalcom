...
const GiftCardDetailPage = ({ onNavigate, onSignUp }) => {
  ...
  const handleBack = () => {
    window.history.pushState({ page: 'gift-card' }, '', '/gift-card');
  ...
  const getAllMedia = () => {
    ...
    // Add all images (up to 5)
    if (giftCard.image_url) media.push({ type: 'image', url: giftCard.image_url });
    if (giftCard.image_url_2) media.push({ type: 'image', url: giftCard.image_url_2 });
    if (giftCard.image_url_3) media.push({ type: 'image', url: giftCard.image_url_3 });
    if (giftCard.image_url_4) media.push({ type: 'image', url: giftCard.image_url_4 });
    if (giftCard.image_url_5) media.push({ type: 'image', url: giftCard.image_url_5 });
    
    // Add all videos (up to 5)
    if (giftCard.video_url) media.push({ type: 'video', url: giftCard.video_url });
    if (giftCard.video_url_2) media.push({ type: 'video', url: giftCard.video_url_2 });
    if (giftCard.video_url_3) media.push({ type: 'video', url: giftCard.video_url_3 });
    if (giftCard.video_url_4) media.push({ type: 'video', url: giftCard.video_url_4 });
    if (giftCard.video_url_5) media.push({ type: 'video', url: giftCard.video_url_5 });
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading gift card...</Text>
        </View>
      </View>
    );
  }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity style={styles.backButton} onPress={handleBack}>
            <Text style={styles.backButtonText}>← Back to Gift Cards</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
                    ...
                    {(() => {
                      ...
                      return currentMedia.type === 'image' ? (
                        ...
                        <video 
                          src={currentMedia.url} 
                          controls 
                          style={{...styles.mainVideoWeb, pointerEvents: 'auto'}} // Keep controls clickable
                        />
                      );
                    })()}
                  </View>
            ...
            <View style={styles.detailsSide}>
              <View style={styles.infoCard}>
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>GIFT CARD</Text>
                </View>
                <Text style={styles.productPrice}>${giftCard.price}</Text>
                <Text style={styles.productTitle}>{giftCard.name}</Text>
                
                <View style={styles.featureList}>
                  <View style={styles.featureItem}>
                    <Text style={styles.featureIcon}>✅</Text>
                    <Text style={styles.featureText}>Instant Digital Delivery</Text>
                  </View>
                  <View style={styles.featureItem}>
                    <Text style={styles.featureIcon}>✅</Text>
                    <Text style={styles.featureText}>High-Quality Image Deliverable</Text>
                  </View>
                  <View style={styles.featureItem}>
                    <Text style={styles.featureIcon}>✅</Text>
              ...
              <View style={styles.descriptionSection}>
                ...
                <Text style={styles.descriptionText}>
                  Upon successful purchase, you will receive a high-quality digital gift card image. This image can be downloaded directly from your "My Downloads" page and can be used for online shopping or gifted to someone special.
                </Text>
              </View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  productTitle: { fontSize: 28, fontWeight: '700', color: '#1D1D1F', marginBottom: 20 },
