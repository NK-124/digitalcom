"""
Migration script to add video_url column to gift_cards table
"""
import os
...
def migrate():
    """Add video_url column to gift_cards table if it doesn't exist"""
    ...
    if SQLALCHEMY_DATABASE_URL.startswith("postgresql"):
        ...
        with engine.connect() as conn:
            ...
            if 'video_url' not in columns:
                print("Adding video_url column to gift_cards table...")
                conn.execute(text(
                    "ALTER TABLE gift_cards ADD COLUMN video_url VARCHAR NULL"
                ))
                conn.commit()
